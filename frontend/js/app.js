...
// MODIFIED: Now goes to difficulty selection instead of direct assessment
    ...
    try {
        ...
        if (subjectsGrid) {
            ...
            subjects.forEach(subject => {
                ...
                `;
                // CHANGED: Now goes to difficulty selection
...
// NEW: Select difficulty and start assessment
...
// NEW: Show AI generation badge
function showAIBadge() {
    const assessmentScreen = document.getElementById('assessment-screen');
    if (assessmentScreen && !document.getElementById('ai-badge')) {
        const badge = document.createElement('div');
        badge.id = 'ai-badge';
        badge.className = 'ai-generated-badge';
        badge.innerHTML = '🤖 AI-Generated Questions';
        
        const container = assessmentScreen.querySelector('.container');
        const questionContainer = assessmentScreen.querySelector('.question-container');
        
        if (container && questionContainer) {
            container.insertBefore(badge, questionContainer);
        }
    }
}
...
// MODIFIED: Now includes difficulty parameter
async function startAssessmentWithDifficulty(subjectId, subjectName, difficulty) {
    try {
        ...
        const data = await apiCall('/api/assessment/start', 'POST', {
            ...
            subject: subjectId,
            difficulty: difficulty // NEW: Pass difficulty to backend
        });
        ...
        // Show AI generation badge if questions were AI-generated
...
// Keep original for backward compatibility (defaults to medium)
...
// MODIFIED: Now includes difficulty selection
...
// NEW: Show difficulty selection for subtopic quiz
...
// NEW: Start subtopic quiz with difficulty
async function startSubtopicQuizWithDifficulty(topicId, subtopicId, subtopicName, difficulty) {
    try {
        ...
        const data = await apiCall('/api/quiz/subtopic', 'POST', {
            ...
            difficulty: difficulty // NEW: Pass difficulty
        });
        ...
        if (data.generatedByAI) {
            console.log('🤖 Subtopic questions generated by AI');
            showSubtopicAIBadge();
        }
...
// NEW: Show AI badge for subtopic quiz
function showSubtopicAIBadge() {
    const quizScreen = document.getElementById('subtopic-quiz-screen');
    if (quizScreen && !document.getElementById('subtopic-ai-badge')) {
        const badge = document.createElement('div');
        badge.id = 'subtopic-ai-badge';
        badge.className = 'ai-generated-badge';
        badge.innerHTML = '🤖 AI-Generated Questions';
        
        const container = quizScreen.querySelector('.container');
        const questionContainer = quizScreen.querySelector('.question-container');
...
function continueSubject(subject) {
    ...
    if (hasAssessment) {
        ...
        // Go to difficulty selection instead of direct assessment
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    console.log('✨ Gemini AI-Powered Question Generation Enabled');
    showWelcome();
});

console.log('✅ app.js loaded successfully with AI features');